...
| `fill_height(state(<amount_field>), capacity_<unit>=<number>)` | a numeric material-amount `state_field` (typically a `float` named `material_volume`, `held_material_volume`, etc.); `<unit>` is `ul`, `ml`, or `mg` and must match the field unit | bind material amount to compiled material-SVG gravity parts using the matching capacity, or to generated geometry for an intentional structured-subpart binding, as defined in [MATERIAL_CONVENTION.md](MATERIAL_CONVENTION.md) |
...
- An object-level material identity/amount binding may use `fill_height` with
  ...
  matching `capacity_ul`, `capacity_ml`, or `capacity_mg` value. Those compiler inputs drive the selected
