...
test("a loaded static SVG image is real and supplies the visual bounding box", () => {
  ...
  assert.deepEqual(classifyRenderedItem(snapshot), {
...
test("explicit render errors override successfully rendered artwork", () => {
  const snapshot = makeSnapshot({
    renderErrorKind: "missing-object",
    hasInlineSvg: true,
...
test("an item with no graphic host is diagnosed as a missing object", () => {
  assert.deepEqual(classifyRenderedItem(makeSnapshot()), {
    hasRenderError: true,
    renderErrorKind: "missing-object",
  });
});
